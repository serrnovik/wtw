function Show-WtwCommandHelp {
    ...
    $help = switch ($Command) {
        'init'        { @(
            ...
            '  --emoji <prefix>            SourceGit / list / cmux / wmux / T3 prefix (e.g. 🎸 or "🎭 ☸️")',
        ...
        'add'         { @(
            ...
            'name, identity emoji, and cmux / Codex / Superset / SourceGit / agentctl',
            ...
            '  --emoji <glyph>   Override the derived identity emoji (pass none / auto for derived)',
        ...
        'create'      { @(
            ...
            '  --emoji <glyph>     Override the derived identity emoji (e.g. 🦔). none / auto = derived',
        ...
        'import'      { @(
            ...
            'The name is the worktree on the other machine. When several local',
            'clones share that remote, import uses the one wtw tracks under that',
            'repo name or alias. --repo names a clone explicitly.',
            'Search ignores case when only one worktree matches.',
            '',
            'Arguments:',
            ...
            '  --repo <name>     Local clone to receive the worktree when several share',
            '                    that git remote.',
        ...
        { $_ -in 'edit', 'rename', 'ren' } {
            @(
                ...
                '  --emoji <glyph>   Override the derived identity emoji. none / auto restores derived',
                '',
                'Repo options:',
                ...
                '  --emoji <prefix>  Repo: SourceGit / list / cmux / wmux / T3 prefix',
                '                    Worktree: identity glyph override. Pass - / none / auto to clear',
                '  --sourcegit-folder / --no-sourcegit-folder',
                ...
                '  wtw edit snowmain1 --emoji 🎸     Prefix SourceGit / list / cmux / wmux names',
                '  wtw edit auth --emoji 🦔           Override the worktree identity glyph',
        ...
        { $_ -in 'cmux', 'cm' } {
            @(
                'wtw cmux [name]',
                'Open a target as a local cmux workspace (alias: cm).',
                '',
                'Arguments:',
                '  name    Target to open (default: detected from cwd)',
                '',
        ...
        { $_ -in 'wmux', 'wm' } {
            @(
                'wtw wmux [name]',
                'Open a target as a wmux workspace on Windows (alias: wm).',
                '',
                'Arguments:',
                '  name    Target to open (default: detected from cwd)',
                '',
