function Show-WtwCommandHelp {
    ...
    $help = switch ($Command) {
        'init'        { @('wtw init [aliases...]', 'Register current repo with wtw.', '', 'Options:', '  --template <alias|path>     Share workspace settings from another repo or file', '  --startup-script <name>     Script to run on session entry (overrides auto-detect)', '  --workspaces-dir <path>     Override workspace files directory', '  --name <key>                Override the registry key') }
